...
def mock_page():
    """创建模拟的 DrissionPage"""
    page = MagicMock()
    page.tab_ids = MagicMock(return_value=['tab-1'])
    page.tab_id = MagicMock(return_value='tab-1')
    page.new_tab = MagicMock()
    page.close = MagicMock()
    page.to_tab = MagicMock()
    return page
...
class TestTabManagerInit:
    ...
    def test_tabs_property_returns_tab_ids(self, tab_manager, mock_page):
        ...
        mock_page.tab_ids.return_value = ['tab-1', 'tab-2']
...
class TestCloseTab:
    ...
    def test_close_tab_raises_on_last_tab(self, tab_manager, mock_page):
        """测试关闭最后一个标签页时抛出异常"""
        mock_page.tab_ids.return_value = ['tab-1']
    ...
    def test_close_tab_switches_before_close(self, tab_manager, mock_page):
        """测试关闭非当前标签页前先切换"""
        mock_page.tab_ids.return_value = ['tab-1', 'tab-2']
        mock_page.tab_id.return_value = 'tab-2'
        tab_manager.close_tab(0)
        mock_page.to_tab.assert_called_with(0)
...
class TestSwitchToTab:
    ...
    def test_switch_to_tab_success(self, tab_manager, mock_page):
        ...
        mock_page.to_tab.assert_called_with(1)
...
class TestTabsCount:
    ...
    def test_tabs_count_empty(self, tab_manager, mock_page):
        """测试空状态时标签页数量为 0"""
        mock_page.tab_ids.return_value = []
    ...
    def test_tabs_count_multiple(self, tab_manager, mock_page):
        """测试多个标签页数量正确"""
        mock_page.tab_ids.return_value = ['tab-1', 'tab-2', 'tab-3']
