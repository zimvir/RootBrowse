...
Python浏览器自动化 MCP 框架，为 AI Agent 提供结构化的浏览器交互能力。基于 DrissionPage 构建。

## 核心特性

- **全 JS 操作** — 所有元素操作（点击、悬停、双击、右键）都通过 JS 实现，支持隐藏元素
- **直接 xpath 操作** — 不依赖快照式 ref，所有定位直接查实时 DOM，动态页面不失效
- **独立浏览器实例** — 每个进程自动分配独立端口，多 Agent 并行不干扰
...
# 执行操作
result = browser.operation.click(ele.xpath)
...
Operation 操作           → 点击、输入、悬停等（使用 xpath）
```
...
# 直接用 xpath 点击
browser.operation.click(elements[0].xpath)
```

### 独立浏览器实例

每个进程自动分配独立端口，多个 MCP 进程并行运行时互不干扰：

```python
# 进程 A → 端口 9600（独立浏览器）
# 进程 B → 端口 9601（独立浏览器）
browser = Browser()  # auto_port() 自动分配
```
...
```python
browser.operation.click(xpath)              # 点击
browser.operation.input_by_xpath(xpath, text) # 输入
browser.operation.hover(xpath) # 悬停
browser.operation.double_click(xpath)       # 双击
browser.operation.right_click(xpath)         # 右键
browser.operation.submit(xpath)              # 提交表单
browser.operation.clear(xpath)               # 清空输入框
browser.operation.send_enter() # 发送回车
```
...
- **MCP 框架**:官方 Python MCP SDK (`mcp>=1.0.0`)
- **Python 版本**: >= 3.10
